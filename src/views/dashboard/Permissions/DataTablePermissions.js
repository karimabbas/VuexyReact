...
// ** Table Data & Columns
import { selectThemeColors, handlerArabicSearch } from '@utils'
...
import { getEmployeeList, getData, getEmployeePermissions, triggerPermission } from './store/action/index'
const customStyles = {
  cells: {
    style: {
...
const DataTablePermissions = (props) => {
  ...
  const [employee, setEmployee] = useState({ label: 'اختر موظف', value: '' })
  const [currentFilter, setCurrentFilter] = useState({ label: 'الكل', value: '' })
  const searchValueRef = useRef()
  const dispatch = useDispatch()
  const employeePermissions = useSelector(state => state.employeePermissions)
  ...
  const handleFilterCustom = value => {
    ...
    updatedData = employeePermissions?.filteredData?.filter(item => {
  ...
  useEffect(() => {
    ...
    setTableConfig(prev => {
      return { ...prev, loading: false, filteredData: employeePermissions?.filteredData, searchValue: search_value }
    })
    handleFilterCustom(search_value)
  }, [employeePermissions?.filteredData])
  ...
  const handleSwitch = permission => {
    dispatch(triggerPermission(employee, permission, currentFilter))
  }
  const customSort = (rows, selector, direction) => {
    const arrayFilter = tableConfig?.searchValue !== '' ? tableConfig?.filteredData : employeePermissions?.filteredData
  ...
  return (
    <Fragment>
      {employeePermissions.loading ? (
        <Modal isOpen={employeePermissions.loading} className='modal-dialog-centered custom-spinner-modal'>
      ...
      <Card>
        <CardHeader>
          <CardTitle tag='h4'>الصلاحيات</CardTitle>
        </CardHeader>
        <CardBody>
          <Row>
            <Col md='4'>
              <FormGroup>
                <Label for='employee'>الموظفين</Label>
                <Select
                  isClearable={false}
                  ...
                  options={employeePermissions.employeeList}
                  value={employee}
                  onChange={data => {
                    setEmployee(data)
                    setTableConfig(prev => {
                    ...
                    dispatch(
                      getEmployeePermissions(data, currentFilter)
                    )
                  }}
                />
              </FormGroup>
            ...
            <Col md='4'>
              <FormGroup>
                ...
                <Select
                  ...
                  className='react-select'
                  classNamePrefix='select'
                  options={[{ label: 'الكل', value: '' }, ...employeePermissions?.headers ?? []]}
                  value={currentFilter}
                  onChange={data => {
