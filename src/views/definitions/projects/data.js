...
export const projectColumns = (def, handleDeleteDefintion) => [
    ...
    {
        name: 'تاريخ بداية المشروع',
        // selector: row => row.dateStart[0][1][2],
        sortable: true,
        center: true,
        minWidth: '120px',
        cell: row => {

            const year = +(row.dateStart[2])
            const month = +(row.dateStart[1])
            const day = +(row.dateStart[0])

            return [day, '/', month, '/', year]
        }
    },
    {
        name: 'تاريخ نهاية المشروع',
        // selector: 'dateEnd',
        center: true,
        sortable: true,
        minWidth: '120px',
        cell: row => {
            ...
            const year = +(row.dateEnd[2])
            const month = +(row.dateEnd[1])
            const day = +(row.dateEnd[0])
