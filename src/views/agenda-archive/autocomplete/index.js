...
const AutocompleteMain = props => {
  ...
  const onSuggestionItemClick = (url, e) => {
    ...
    setDebouncedTerm(filteredData[activeSuggestion].modelName)
  ...
  const onKeyDown = e => {
    ...
    if (e.keyCode === 38 && activeSuggestion !== 0) {
      ...
      // ** User Pressed ENTER
      props.handleListItemClick(filteredData[activeSuggestion])
      onSuggestionItemClick(filteredData[activeSuggestion].link, e)
      setDebouncedTerm(filteredData[activeSuggestion].name)
      setShowSuggestions(false)
    } else {
      return
    }
      ...
      if (!customRender) {
        ...
        return (
          <li
            ...
            key={`${item[filterKey]}${item.id}`}
            onClick={e => {
              onSuggestionItemClick(suggestionURL, e)
              props.handleListItemClick(item)
            }}
            onMouseEnter={() => {
            ...
            >
              <div className='item-container d-flex'>
                <div className='item-info ml-1'>
                  <p className='align-center mb-0'>{item[filterKey]}</p>
                </div>
              </div>
            </div>
          </li>
  ...
  const renderSuggestions = () => {
    const { filterKey, filterHeaderKey, suggestions, loading } = props
    if (loading) {
      return (
  ...
  if (showSuggestions) {
    suggestionsListComponent = (
      <PerfectScrollbar
        className={classnames('suggestions-list-meeting', {
